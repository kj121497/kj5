...
The process of testing a low-fidelity was somewhat difficult, as the prototype is not interactive and the features I implemented are not necessarily visible to another person when they are from my point of view. Nevertheless, the user was able to follow almost all of the steps correctly, even though he struggled to move from wireframe to wireframe—as he had to observe each wireframe closely to be able to move forward. This makes me think that low-fidelity prototyping requires more attention span and observation from users compared to high-fidelity prototyping. As mentioned in “Test”, the user had trouble understanding the search bar only appears when the screen is pulled down, the user was unable to identity the newly implemented search bar, and he did not understand a video must be tapped so the play/pause screen appears. Based on the feedback I received, I would make it easier to understand that the search bar for searching profile posts needs to be pulled down in order to be accessed (perhaps, this could be its own wireframe and may even require multiple wireframes). Also, I would make the search bar larger in size when it is pulled down, as the user was unable to identify it in Wireframe 2. Furthermore, I would make it easier to understand that a video must be tapped in order to display the play/pause screen (as aforementioned, this would probably need to be illustrated through multiple wireframes).
